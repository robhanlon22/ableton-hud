...
function statusIcon(status: StatusKind): React.JSX.Element {
  if (status === 'playing') {
    return <Play className="h-3.5 w-3.5 fill-current" aria-hidden />;
  }
  if (status === 'stopped') {
    return <Square className="h-3 w-3 fill-current" aria-hidden />;
  }
  return <WifiOff className="h-3.5 w-3.5" aria-hidden />;
}

function statusVariant(state: HudState): 'neutral' | 'success' | 'warning' | 'offline' {
...
function metadataPillStyle(color: number | null): CSSProperties | undefined {
  if (color === null) {
    return undefined;
  }
...
export function shouldHoldNullClipTransition(previous: HudState, next: HudState): boolean {
  if (!next.isPlaying || next.clipIndex !== null || previous.clipIndex === null) {
    return false;
  }
...
type ClipColorState = Pick<HudState, 'isPlaying' | 'trackIndex' | 'clipIndex' | 'clipColor'>;

export function resolveHeldClipColor(previousHeldColor: number | null, nextState: ClipColorState): number | null {
...
export function HudSurface({
  ...
  const flashClass = useMemo(() => panelFlashClass(state, isFlashActive), [state, isFlashActive]);
            ...
            <div className="grid min-w-0 flex-1 grid-cols-3 gap-1">
              <div
                className={cn(
                  ...
                  state.clipColor === null ? 'border-[#5a616d] bg-[#2a3037] text-ableton-text' : 'border-transparent'
                )}
                style={clipStyle}
              ...
              <div
                className={cn(
                  'flex h-6 min-w-0 items-center justify-center rounded-[4px] border px-1 font-ui text-[11px] font-medium leading-none',
                  state.trackColor === null ? 'border-[#5a616d] bg-[#2a3037] text-ableton-text' : 'border-transparent'
                )}
                style={trackStyle}
              ...
              <div
                className={cn(
                  'flex h-6 min-w-0 items-center justify-center rounded-[4px] border px-1 font-ui text-[11px] font-medium leading-none',
                  state.sceneColor === null ? 'border-[#5a616d] bg-[#2a3037] text-ableton-text' : 'border-transparent'
                )}
                style={sceneStyle}
        ...
        <CardFooter className="pt-1">
          <div className="flex w-full items-center gap-2">
            ...
            <Button
              ...
              aria-label={state.alwaysOnTop ? 'Set window normal' : 'Set window floating'}
...
export function HudApp(): React.JSX.Element {
  const [hudState, setHudState] = useState<HudState>(() => createDefaultHudState());
  const [isFlashActive, setIsFlashActive] = useState(false);
  const [heldClipColor, setHeldClipColor] = useState<number | null>(null);
  ...
  const pendingNullClipTimerRef = useRef<number | null>(null);

  useEffect(() => {
    setHeldClipColor((previousHeldColor) => resolveHeldClipColor(previousHeldColor, hudState));
  }, [hudState.isPlaying, hudState.trackIndex, hudState.clipIndex, hudState.clipColor]);
  ...
  const onToggleMode = (): void => {
    const nextMode: HudMode = hudState.mode === 'elapsed' ? 'remaining' : 'elapsed';
  ...
  const renderState = useMemo(() => {
    const hasActiveClip = hudState.trackIndex !== null && hudState.clipIndex !== null;
    const clipColor = hasActiveClip ? (hudState.clipColor ?? heldClipColor) : null;
